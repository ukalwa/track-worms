...
folder = uigetdir;
filenames = find_files_recursive(folder,'\w*worm+\w*.xls');

GRAY_SCALE_PLOT = true;
h = waitbar(0,'Processing files...');
emp = cell(10000,100);
count = zeros(599,1);
for i = 1:length(filenames)
    disp(filenames{i});
    data = xlsread(filenames{i},'Sheet1');
    if(size(data,1)>1200)
        data = data(1:1200,:);
    end
...
%     saveas(plot_curvature(k(1:10:end,:),0,0,0), fullfile(path,[name,'_plot.png']));
    if GRAY_SCALE_PLOT
        plot_extension_name = '_plot_grayscale.png';
    else
        plot_extension_name = '_plot_2.png';
    end
    saveas(plot_curvature(k(1:10:end,:),0,0,0, GRAY_SCALE_PLOT), ...
            fullfile(path,[name, plot_extension_name]));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
